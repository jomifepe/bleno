...
Hci.prototype.setAdvertisingParameters = function() {
  ...
  var advInt = process.env.BLENO_ADVERTISING_INTERVAL
  var advertisementInterval = Math.floor(
    parseFloat(advInt && advInt > 19 && advInt < 10001 ? advInt : 500) * 1.6);

  // data
  cmd.writeUInt16LE(advertisementInterval, 4); // min interval
  cmd.writeUInt16LE(advertisementInterval, 6); // max interval
  cmd.writeUInt8(process.env.BLENO_CONNECTABLE_MODE == 1 ? 0x00 : 0x03, 8); // adv type
